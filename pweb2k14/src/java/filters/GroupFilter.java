...
public class GroupFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            User usr = ServletHelperClass.getUserFromSession((HttpServletRequest)request);
            helper = (DbHelper) getFilterConfig().getServletContext().getAttribute("dbmanager");

            if(uri.contains("listPosts.jsp"))
            {
                if(((HttpServletRequest) request).getSession().getAttribute("group") == null)
                ...
                else
                {
                    ...
                    if(!helper.doesUserBelongsToGroup(usr, g))
                    {
                        if(usr != null && usr.isIsmoderator())
                            ...
                            ((HttpServletResponse)response).sendRedirect("../NotAllowed.jsp");   
